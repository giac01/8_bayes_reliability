...
::: {.callout-note}
This tutorial can be run and tested inside the docker image `bignardig/tidyverse461:v5`, which has all the required R packages pre-installed.
:::
...
`library(gbtoolbox)` is already loaded above (see [Load Data]), but if you don't have it installed, you can either install my R package of miscellaneous R functions `gbtoolbox` available on github, or you can just load the reliability function directly from github (not run here, since `gbtoolbox` is already installed in the docker image used to build this tutorial). Both methods are outlined below:
...
RMU estimated using Time 1 data:
...
RMU estimated using Time 2 data:
