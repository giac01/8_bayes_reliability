...
This is a brief walk-through of how to calculate reliability using the approach (relative measurement uncertainty; RMU) described in our [pre-print](www.addlink.com). 
...
data_long_summarised %>%
  ...
  kableExtra::add_header_above(c("Unique Participant ID" = 1, "Testing Session" = 1, "Condition (contrast coded)" = 1, "Number of Trials" = 1, "Number of Go Responses in Condition" = 1))
...
![RMU reliability estimation process](plots/9_posteriors.png)
...
The analyses indicate that reliability at the first time point was `r round(rmu_time1$hdci$y*100)`% (95% Credible Interval = `r round(rmu_time1$hdci$ymin*100)`%-`r round(rmu_time1$hdci$ymax*100)`%), and at the second time point reliability was `r round(rmu_time2$hdci$y*100)`% (95% Credible Interval = `r round(rmu_time2$hdci$ymin*100)`%-`r round(rmu_time2$hdci$ymax*100)`%).
