...
sim_factor_s = function(
  ...
  dat         = sapply(1:n_items, function(i) std_loading[i]*true_scores + rnorm(n_rows, sd = sqrt(error_variance[i]))) # Note the sd(true_score) and sd(error) must be kept at 1 for the loading calculation to make sense!
