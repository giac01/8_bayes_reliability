...
To calculate RMU, extract the posterior draws for each mouse's random intercept parameter, and plug the matrix of draws into the reliability function:
...
$$
\begin{aligned}
\hat{y}_i &= \frac{1}{J}\sum_{j=1}^{J} y_{ij} \\
&= \frac{1}{J}\sum_{j=1}^{J} (\theta_i + \epsilon_{ij}) \\
&= \theta_i + \sum_{j=1}^{J} \frac{\epsilon_{ij}}{J}  \\
&= \theta_i + \mathcal{E}_i   \\
\mathcal{E}_i &=\frac{\epsilon_{i1} + \epsilon_{i2} + \dots + \epsilon_{iJ}}{J} \sim \text{Normal}(0,\frac{ \sigma^2_{\epsilon} }{J})
\end{aligned}
$$

To work out the variance of the mean score for subject *i*, note that the variance of the sum of J independent normally distributed variables ($\epsilon_{i1} + \epsilon_{i2} + \dots + \epsilon_{iJ}$) is equal to $J\sigma^2_{\epsilon}$, and dividing by a constant J [divides the variance](https://en.wikipedia.org/wiki/Variance#Addition_and_multiplication_by_a_constant) by $J^2$.
...
$$
\begin{aligned}
R(\hat{\mathbf{y}}) &= R(\mathbf{\theta}+\mathcal{E})\\
&=\frac{Var(\mathbf{\theta})}{Var(\mathbf{\theta})+Var(\mathcal{E})} \\
&= \frac{\sigma^2_{\theta}}{\sigma^2_{\theta}+\sigma^2_{\epsilon}/J}
\end{aligned}
$$
