...
`library(gbtoolbox)` is already loaded above (see [Set Up]), but if you don't have it installed, you can either install my R package `gbtoolbox` from github, or you can just load the reliability function directly from github (not run here, since `gbtoolbox` is already installed in the docker image used to build this tutorial). Both methods are outlined below:
...
gbtoolbox::reliability(posterior_draws)
