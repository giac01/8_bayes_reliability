...
// V6: Moved back to normal parametisation, clarifed some terms
...
transformed parameters {
  ...
  learning_rate  = Phi_approx(mu[1] + sigma[1]*learning_rate_z);    
  decision_noise = 5 * Phi_approx(mu[2] + sigma[2]*decision_noise_z);
}

model {
  // Hyperparameters
  mu    ~ normal(0, 1);               // mu_z[1] -> average pop learning rate 
  sigma ~ normal(0, 0.2);             // standard deviations in individual diff in learning rate and decision noise

  // individual parameters
  learning_rate_z  ~ normal(0, 1);              // Individual differences in learning rate (standardized scale)
  decision_noise_z ~ normal(0, 1);              // Individual differences in decision noise / temperature (standardized scale)
